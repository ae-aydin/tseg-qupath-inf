...
def predict(
    config: dict, args: argparse.Namespace, operation: str, k_size: int
):
    full_shape = (int(args.roi_width), int(args.roi_height))
    logging.info(f"Selected region size is {format_shapes(full_shape)}")

    w = int(np.ceil(args.roi_width / args.ds))
    h = int(np.ceil(args.roi_height / args.ds))
    logging.info(f"Downsampled region size is {format_shapes((w, h))}")

    main_mask = np.zeros((h, w), dtype=np.uint8)
    model = YOLO(config["model_path"])
    logging.info(f"Model arguments - confidence: {args.conf}, iou: {args.iou}, imgsz: {args.imgsz}")
    ...
    logging.info(f"Mask prepared in {time.time() - mask_start:.2f} seconds.")
